...
class Requisicion extends Conexion{
        ...
        if($row=mysql_fetch_array($res)){
            ...
            $query="INSERT INTO Alm_requi(Consecutivo,Fr,Ur,Estadodoc,Festadodoc,Sucursal,Centrocosto)
                VALUES($nuevo,now(),'".$_SESSION["Cedula"]."','".$estado."',now(),".$_SESSION["Id_sucursal"].",
                    ".$_SESSION["Id_centro"].");";
           mysql_query($query);
           echo "<script>location.href='".Conexion::ruta()."Requisicion?b=".$nuevo."'</script>";
        }
    }
    ...
    public function get_requi($con){
        ...
        // echo $sql="SELECT Centros.Nombre AS Centro,Requi.Consecutivo,Requi.Id,Requi.Fr,Requi.Observaciones,Requi.Fentrega,
        ...
        $sql="SELECT Centros.Nombre Centro,Requi.Consecutivo,Requi.Id,Requi.Fr,Requi.Observaciones,
        Requi.Fentrega,Requi.Lentrega,Requi.Estadoproceso,(SELECT Lista 
                                                     FROM G_listas
                                                     WHERE Id_control=1
                                                     AND Numeracion=Requi.Estadodoc) as Estadodoc,
            Requi.Ps,Requi.Estadodoc Id_estado,Sucursal.Nombre Sucursal
            FROM Alm_requi Requi
            JOIN G_centros_costo Centros ON Requi.Centrocosto=Centros.Id
            JOIN G_sucursales AS Sucursal ON Requi.Sucursal=Sucursal.Id
            WHERE Requi.Consecutivo=$con 
            AND Requi.Centrocosto={$_SESSION["Id_centro"]}
        ";
